function [obj,pset,qset,vo_set_droop] = ibrGFMValInitializations(obj, ...
...
for idxIbrGFM = 1:obj.numIbrGFM
    ...
    iabc(:,idxIbrGFM) = real([ioPhA;ioPhB;ioPhC])* ...
    ...
    vo_set_droop(idxIbrGFM) = vo_dq(2,idxIbrGFM);
...
% Save in x_v
idxIbrGFM = obj.getRowIdx_fromIbrGFMIndices(1:obj.numIbrGFM) - 1;
...
obj.x_v(idxIbrGFM+idxIabc(1)) = iabc(1,:).';
obj.x_v(idxIbrGFM+idxIabc(2)) = iabc(2,:).';
obj.x_v(idxIbrGFM+idxIabc(3)) = iabc(3,:).';
% Passive
obj.x_v(idxIbrGFM+idxVg(1)) = vg_dq(1,:).';
obj.x_v(idxIbrGFM+idxVg(2)) = vg_dq(2,:).';
obj.x_v(idxIbrGFM+idxVo(1)) = vo_dq(1,:).';
obj.x_v(idxIbrGFM+idxVo(2)) = vo_dq(2,:).';
obj.x_v(idxIbrGFM+idxIf(1)) = if_dq(1,:).';
obj.x_v(idxIbrGFM+idxIf(2)) = if_dq(2,:).';
obj.x_v(idxIbrGFM+idxIo(1)) = io_dq(1,:).';
obj.x_v(idxIbrGFM+idxIo(2)) = io_dq(2,:).';
obj.x_v(idxIbrGFM+idxVi(1)) = vi_dq(1,:).';
obj.x_v(idxIbrGFM+idxVi(2)) = vi_dq(2,:).';
% Current control
obj.x_v(idxIbrGFM+idxGamma(1)) = gamma_dq(1,:).';
obj.x_v(idxIbrGFM+idxGamma(2)) = gamma_dq(2,:).';
% Voltage control
obj.x_v(idxIbrGFM+idxIfRef(1)) = if_dq_ref(1,:).';
obj.x_v(idxIbrGFM+idxIfRef(2)) = if_dq_ref(2,:).';
obj.x_v(idxIbrGFM+idxX(1)) = x_dq(1,:).';
obj.x_v(idxIbrGFM+idxX(2)) = x_dq(2,:).';
% Droop
obj.x_v(idxIbrGFM+idxWr) = wr;
obj.x_v(idxIbrGFM+idxVoRefDroop_q) = voDroop_q_ref;
obj.x_v(idxIbrGFM+idxPqAvg(1)) = pqavg(1,:).';
obj.x_v(idxIbrGFM+idxPqAvg(2)) = pqavg(2,:).';
obj.x_v(idxIbrGFM+idxTheta) = THETA;
% Virtual resistor
obj.x_v(idxIbrGFM+idxVo_VR_avg(1)) = vo_dq_VR_avg(1,:);
obj.x_v(idxIbrGFM+idxVo_VR_avg(2)) = vo_dq_VR_avg(2,:);
obj.x_v(idxIbrGFM+idxVo_VR(1)) = vo_dq_VR(1,:);
obj.x_v(idxIbrGFM+idxVo_VR(2)) = vo_dq_VR(2,:);
obj.x_v(idxIbrGFM+idxVoRef(1)) = vo_dq_ref(1,:);
obj.x_v(idxIbrGFM+idxVoRef(2)) = vo_dq_ref(2,:);
